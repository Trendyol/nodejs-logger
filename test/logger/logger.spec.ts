...
import { Adapter, AdapterLog, Detail, GraylogConfig, LogContext, LogLevel } from '../../src/types';
...
describe('logger specs', () => {
  ...
  describe('without log level config', () => {
    beforeEach(() => {
  ...
  describe('when log level config is error ', () => {
    beforeEach(() => {
      mockAdapter = sandbox.stub(adapter);

      mockAdapter.validate.returns(true);

      logger = new Logger({ actions, adapters: [mockAdapter], logLevel: LogLevel.error });
    });
  ...
  describe('when log level config is debug ', () => {
    beforeEach(() => {
      mockAdapter = sandbox.stub(adapter);

      mockAdapter.validate.returns(true);

      logger = new Logger({ actions, adapters: [mockAdapter], logLevel: LogLevel.debug });
    });
  ...
  describe('when log level config is info ', () => {
    beforeEach(() => {
      mockAdapter = sandbox.stub(adapter);

      mockAdapter.validate.returns(true);

      logger = new Logger({ actions, adapters: [mockAdapter], logLevel: LogLevel.info });
    });
  ...
  describe('when log level config is warn ', () => {
    beforeEach(() => {
      mockAdapter = sandbox.stub(adapter);

      mockAdapter.validate.returns(true);

      logger = new Logger({ actions, adapters: [mockAdapter], logLevel: LogLevel.warn });
    });
